...
const StyledCustomWatchVideo = styled.div`
  display: grid;
  align-items: center;
...
const StyledCustomWatchVideoImg = styled.div<{
  $videoImgUrl: ICustomWatchVideo["videoImgUrl"];
}>`
  position: relative;
...
const StyledCustomWatchVideoWrapper = styled.div`
  display: flex;
  flex-direction: column;
  justify-content: center;
`;

const StyledCustomWatchVideoHeading = styled(Heading)`
  margin-bottom: 8px;
...
const StyledCustomWatchVideoLabel = styled.button`
  border: none;
  font-size: 16px;
  line-height: 21px;
  color: #ff6f3d;
...
const StyledCustomWatchVideoModal = styled.div`
  position: relative;
